...
mod tests {
    ...
    fn service_form_unregistered_key() {
        ...
        assert_eq!(result.name, DomainName::try_from("test.example.com").unwrap());
        assert_eq!(result.parameters.len(), 1);
        if let ServiceParameter::PRIVATE { number, presentation_key, wire_data, presentation_value } = &result.parameters[0] {
    ...
    fn service_form_multiple_parameters() {
        ...
        assert!(matches!(result.parameters[0],
            ServiceParameter::MANDATORY ) ); // TODO not sure how to verify the parameter value here
